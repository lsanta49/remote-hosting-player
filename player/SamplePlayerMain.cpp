...
namespace
{
    ...
    // Chave de storage para o IP do PC
    constexpr wchar_t kPcIpStorageKey[] = L"ReversePcIpAddress";
...
void SamplePlayerMain::ConnectOrListen()
{
    ...
    // ── ADICIONADO: Resolve hostname vazio no modo connect ───────────────────
    if (!m_playerOptions.m_listen && m_playerOptions.m_hostname.empty())
    {
        winrt::hstring savedIp = LoadSavedPcIp();
        if (!savedIp.empty())
        {
            m_playerOptions.m_hostname = savedIp;
            m_playerOptions.m_ipv6 = (!savedIp.empty() && savedIp.front() == L'[');
        }
        else
        {
            // Sem IP configurado: exibe instrução e não tenta conectar
            UpdateStatusDisplay();
            return;
        }
    }
    ...
    try
    {
        ...
        {
            // Modo listen original (mantido para compatibilidade com -listen arg)
            m_playerContext.Listen(m_playerOptions.m_hostname, port, port + 1);
        }
        else
        {
            // ── MODO REVERSO: HoloLens conecta ativamente ao PC ──────────────
            // ANTES: HoloLens escutava, PC conectava
            // DEPOIS: HoloLens conecta ao IP do PC, PC escuta
            m_playerContext.Connect(m_playerOptions.m_hostname, port);

            // Persiste o IP para a próxima sessão
            SavePcIp(m_playerOptions.m_hostname);
        }
    }
    catch (winrt::hresult_error& ex)
    {
        m_errorHelper.AddError(
            std::wstring(m_playerOptions.m_listen ? L"Failed to Listen: " : L"Failed to Connect: ") +
...
// Lê o IP do PC salvo em ApplicationData.LocalSettings
winrt::hstring SamplePlayerMain::LoadSavedPcIp()
{
    try
    {
        auto localSettings = winrt::Windows::Storage::ApplicationData::Current().LocalSettings();
        auto value = localSettings.Values().TryLookup(kPcIpStorageKey);
        if (value != nullptr)
        {
...
// Persiste o IP do PC em ApplicationData.LocalSettings
void SamplePlayerMain::SavePcIp(const winrt::hstring& ip)
{
    try
    {
        auto localSettings = winrt::Windows::Storage::ApplicationData::Current().LocalSettings();
        localSettings.Values().Insert(kPcIpStorageKey, winrt::box_value(ip));
    }
    catch(...){}
}
...
SamplePlayerMain::PlayerOptions SamplePlayerMain::ParseActivationArgs(const IActivatedEventArgs& activationArgs)
{
        ...
        switch (activationKind)
        {
            ...
            case Activation::ActivationKind::Protocol:
            {
                ...
                if (uri)
                {
                    ...
                    {
                        try { winrt::hstring v = query.GetFirstValueByName(L"stats"); showStatistics = true; }
                        catch (...) {}

                        try { winrt::hstring v = query.GetFirstValueByName(L"listen"); listen = true; }
                        catch (...) {}
                    }
                }
                break;
    ...
    if (argsProvided)
    {
        ...
        // ── ALTERADO: Lógica de fallback quando hostname está vazio ──────────
        // ANTES: Sempre forçava listen = true quando hostname vazio
        //        hostname = L"0.0.0.0"; listen = true;
        //
        // DEPOIS: Só força listen se explicitamente solicitado via -listen.
        //         No modo connect (padrão reverso), ConnectOrListen() vai
        //         tentar carregar o IP salvo via LoadSavedPcIp().
        if (hostname.empty())
        {
            if (listen)
            {
                // Modo listen explícito via -listen arg: comportamento original
                hostname = L"0.0.0.0";
            }
            // else: hostname vazio em modo connect → ConnectOrListen() resolve
        }
...
void SamplePlayerMain::UpdateStatusDisplay()
{
    ...
    if (m_trackingLost)
    {
        StatusDisplay::Line lines[] = {
            StatusDisplay::Line{L"Device Tracking Lost", StatusDisplay::Small, StatusDisplay::Yellow, 1.0f}};
    ...
    else
    {
        if (m_playerContext.ConnectionState() != ConnectionState::Connected)
        {
            // ── ADICIONADO: Estado "sem IP configurado" ───────────────────────
            // Exibido na primeira execução antes de qualquer IP ser fornecido.
            if (!m_playerOptions.m_listen && m_playerOptions.m_hostname.empty())
            {
                StatusDisplay::Line lines[] = {
                    StatusDisplay::Line{L"Holographic Remoting Player", StatusDisplay::LargeBold, StatusDisplay::White, 1.0f},
                    StatusDisplay::Line{L"Reverse Connection Mode", StatusDisplay::Small, StatusDisplay::Yellow, 1.0f},
                    StatusDisplay::Line{L"Provide your PC's IP address to connect.", StatusDisplay::Small, StatusDisplay::White, 5.0f},
                    StatusDisplay::Line{L"Launch via Device Portal with argument:", StatusDisplay::Small, StatusDisplay::White, 1.0f},
                    StatusDisplay::Line{L"[PC-IP-ADDRESS]:8265", StatusDisplay::Medium, StatusDisplay::Yellow},
                    StatusDisplay::Line{L"e.g:  192.168.1.100:8265", StatusDisplay::Small, StatusDisplay::White, 5.0f},
                    StatusDisplay::Line{L"Or via Protocol URI:", StatusDisplay::Small, StatusDisplay::White, 1.0f},
                    StatusDisplay::Line{L"holographic-remoting-player://[PC-IP]", StatusDisplay::Small, StatusDisplay::Yellow},
                };
                m_statusDisplay->SetLines(lines);
                return;
            }

            // Estado normal: listen ou connect com hostname definido
            // (código original mantido — sem alteração)
            StatusDisplay::Line lines[] = {
                StatusDisplay::Line{L"Holographic Remoting Player", StatusDisplay::LargeBold, StatusDisplay::White, 1.0f},
                StatusDisplay::Line{
                    L"This app is a companion for Holographic Remoting apps.",
                    StatusDisplay::Small, StatusDisplay::White, 1.0f},
                StatusDisplay::Line{L"Connect from a compatible app to begin.", StatusDisplay::Small, StatusDisplay::White, 15.0f},
                StatusDisplay::Line{
                    // NOTA: "Connecting to" já era o texto original para modo connect
                    m_playerOptions.m_listen ? L"Waiting for connection on" : L"Connecting to PC at",
                    StatusDisplay::Small, StatusDisplay::White},
            };
            m_statusDisplay->SetLines(lines);

            std::wostringstream addressLine;
            // Em listen: exibe IP do próprio HoloLens
            // Em connect: exibe IP do PC de destino
            addressLine << (m_playerOptions.m_listen
                ? m_deviceIp.c_str()
                : m_playerOptions.m_hostname.c_str());
